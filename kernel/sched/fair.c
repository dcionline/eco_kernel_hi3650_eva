...
int global_boost_enabled_flag = 0;
int boot_boost = 1;
unsigned int sd_capacity_margin = 1280;
unsigned long up_migration_util_filter = 25;
...
energy_diff(struct energy_env *eenv)
{
	...
	/* Return energy diff when boost margin is 0 or negative */
	if (boost <= 0) {
		trace_sched_energy_diff(eenv->task,
				eenv->src_cpu, eenv->dst_cpu, eenv->util_delta,
				eenv->nrg.before, eenv->nrg.after, eenv->nrg.diff,
				eenv->cap.before, eenv->cap.after, eenv->cap.delta,
				eenv->nrg.delta, eenv->payoff, boost);
		return eenv->nrg.diff;
	}
...
static bool need_spread_task(int cpu)
{
	...
	for_each_cpu(i, sched_domain_span(sd)) {
		if (cpu_util(i) * capacity_margin < capacity_orig_of(i) * 1024) {
			return false;
		}
	}
		  ...
		  int this_cpu, int sd_flag)
{
	...
	do {
		...
		for_each_cpu(i, sched_group_cpus(group)) {
			 ...
			 */
			if (capacity_of(i) < fit_capacity && __task_fits(p, i, cpu_util_wake(i, p))) {
			 ...
			 * single cpu.
			 */
			spare_capacity = capacity_of(i) - cpu_util_wake(i, p);
	...
	if (fit_group && (sd->flags & SD_ASYM_CPUCAPACITY))
		return fit_group;
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD

#define CPU_OVERLOADED_THRE 80 /* treat 80% load as overloaded */
static inline bool cpu_overloaded(int cpu, u64 walltime)
{
	unsigned int cpu_load = get_cpu_load(cpu, walltime);

	return cpu_load > CPU_OVERLOADED_THRE;
}

static inline bool task_fits_half_curr(struct task_struct *p, int cpu)
{
	unsigned long capacity = capacity_curr_of(cpu);
	unsigned long util     = boosted_task_util(p);

	return capacity > (util << 1);
}

static inline bool sg_need_check_load(struct sched_group *sg)
{
	return !hisi_test_fast_cpu(cpumask_first(sched_group_cpus(sg)));
}

#endif /* CONFIG_HISI_EAS_CHECK_LOAD */
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
	/* Get walltime for cpu_overloaded() */
	u64 walltime = ktime_to_us(ktime_get());
	bool need_check_load;
	int overloaded_cpu_cnt;
#endif
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
		need_check_load = sg_need_check_load(sg_target);
try_again:
		overloaded_cpu_cnt = 0;
#endif
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
			/*
			 * Avoid placing task on an overloaded cpu to reduce
			 * unnecessary frequency increase, unless task is heavy.
			 */
			if (need_check_load &&
			    cpu_overloaded(i, walltime) && task_fits_half_curr(p, i)) {
				overloaded_cpu_cnt++;
				continue;
			}
#endif
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
		if (need_check_load && overloaded_cpu_cnt && target_cpu < 0) {
			need_check_load = false;
			goto try_again;
		}
#endif
	} else {
	...
	if (target_cpu != task_cpu(p)) {
...
#ifdef CONFIG_HISI_EAS_CHECK_LOAD
		/* Previous cpu is overloaded */
		if (cpu_overloaded(task_cpu(p), walltime) &&
		    !cpu_overloaded(target_cpu, walltime))
			return target_cpu;
#endif
		...
		if (nrg_diff == 0) {
			if (capacity_orig_of(task_cpu(p)) == capacity_orig_of(target_cpu)) {
				if (cpu_util_wake(target_cpu, p) > cpu_util_wake(task_cpu(p), p))
					return task_cpu(p);
...
#ifdef CONFIG_HISI_EAS_SCHED
	if (boot_boost || (global_boost_enabled_flag && (schedtune_task_boost(p) > 0))) {
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	 ...
	 * 1) energy_aware is enabled and small task is not migrated to higher
	 *    capacity CPU
...
next_group:
	 ...
	 */
#ifdef CONFIG_HISI_EAS_SCHED
	if (sds->total_capacity * 1024 < sds->total_util * sd_capacity_margin)
#else
...
static int need_active_balance(struct lb_env *env)
{
	...
	if ((capacity_orig_of(env->src_cpu) < capacity_orig_of(env->dst_cpu)) &&
		...
		if (idle_cpu(env->dst_cpu))
			return 1;

		if (!idle_cpu(env->dst_cpu) && !cpu_overutilized(env->dst_cpu))
			return 1;
	}
